...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"migrate_locked_picklist_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
def run_migration():
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='locked_picklists'")
