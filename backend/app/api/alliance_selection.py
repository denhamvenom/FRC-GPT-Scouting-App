...
async def lock_picklist(request: LockPicklistRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        existing_picklist = db.query(LockedPicklist).filter(
...
async def get_picklists(team_number: Optional[int] = None, event_key: Optional[str] = None, 
...
async def unlock_picklist(picklist_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Picklist successfully unlocked and deleted"
        }
...
async def create_alliance_selection(request: CreateAllianceSelectionRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if request.picklist_id is not None:
            picklist = db.query(LockedPicklist).filter(LockedPicklist.id == request.picklist_id).first()
            if not picklist:
                raise HTTPException(status_code=404, detail="Picklist not found")
            
            # Check if a selection process already exists for this picklist
            existing = db.query(AllianceSelection).filter(AllianceSelection.picklist_id == request.picklist_id).first()
            if existing:
                raise HTTPException(status_code=400, detail="Alliance selection already exists for this picklist")
...
async def get_alliance_selection(selection_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Get team statuses
        team_statuses = db.query(TeamSelectionStatus).filter(
...
async def team_action(request: TeamActionRequest, db: Session = Depends(get_db)):
    ...
    try:
        # Get the selection
        selection = db.query(AllianceSelection).filter(AllianceSelection.id == request.selection_id).first()
        if not selection:
            raise HTTPException(status_code=404, detail="Alliance selection not found")
        ...
        # Get the team status
        team_status = db.query(TeamSelectionStatus).filter(
        ...
        if request.action == "captain":
            ...
            if team_status.is_captain or team_status.is_picked:
                raise HTTPException(status_code=400, detail="Team is already a captain or has been picked")
            ...
            elif (current_round == 2 or 
                 (current_round == 1 and alliance.first_pick_team_number != 0)) and alliance.second_pick_team_number == 0:
                alliance.second_pick_team_number = request.team_number
            elif current_round == 3 and (alliance.backup_team_number == 0 or alliance.backup_team_number is None):
                alliance.backup_team_number = request.team_number
            else:
                raise HTTPException(status_code=400, detail="Alliance already has all picks for current round")
...
async def advance_to_next_round(selection_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if current_round >= 3:
            ...
            db.commit()
            return {"status": "success", "action": "completed", "selection_id": selection_id, "message": "Alliance selection completed successfully!"}
