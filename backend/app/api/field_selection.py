...
async def save_field_selections(request: Request):
    ...
    try:
        ...
        validation_results = validate_field_mapping(field_selections, critical_mappings, robot_groups)
...
def validate_field_mapping(field_selections, critical_mappings, robot_groups):
    ...
    if duplicate_headers:
        warnings.append(f"Headers assigned to multiple robot groups: {', '.join(duplicate_headers)}")
    ...
    if unmapped_headers:
        warnings.append(f"Headers in robot groups are marked as 'ignore': {', '.join(unmapped_headers)}")
