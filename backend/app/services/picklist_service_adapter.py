...
class PicklistServiceAdapter:
    ...
    async def generate_picklist(
        ...
        try:
            ...
            logger.info(f"PicklistServiceAdapter: use_batching={use_batching}, batch_size={batch_size}")
    ...
    async def rank_missing_teams(
        ...
        """
        Rank missing teams using the new service architecture.
        """
        # Extract team numbers from existing picklist
        existing_numbers = {team["team_number"] for team in ranked_teams}
        ...
        # Exclude the existing ranked teams
    ...
    def merge_and_update_picklist(
        ...
        missing_teams: Optional[List[Dict[str, Any]]] = None,
    ) -> List[Dict[str, Any]]:
        """
        ...
        for ranking in sorted(user_rankings, key=lambda x: x["position"]):
            ...
            team_data = None
            all_teams = picklist + (missing_teams or [])
        ...
        # Add missing teams that weren't user-ranked if provided
        if missing_teams:
