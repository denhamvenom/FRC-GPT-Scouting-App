...
async def create_sheet_config(
    ...
    try:
        ...
        return {"status": "error", "message": f"Error creating sheet configuration: {str(e)}"}


async def get_sheet_configurations(
    db: Session, event_key: Optional[str] = None, year: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "error", "message": f"Error getting sheet configurations: {str(e)}"}


async def get_sheet_configuration(db: Session, config_id: int) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not config:
            return {"status": "error", "message": f"Configuration with ID {config_id} not found"}
        ...
        return {"status": "error", "message": f"Error getting sheet configuration: {str(e)}"}
    ...
    try:
        ...
        if not config and event_key and year:
            ...
            year_query = db.query(SheetConfiguration).filter(
                SheetConfiguration.is_active == True, SheetConfiguration.year == year
            )
        ...
        if not config:
            logger.info("No config found with filters, trying any active config")
        ...
        return {"status": "error", "message": f"Error getting active configuration: {str(e)}"}


async def set_active_configuration(
    db: Session, config_id: int, event_key: Optional[str] = None, year: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "error", "message": f"Error setting active configuration: {str(e)}"}
...
async def delete_configuration(db: Session, config_id: int) -> Dict[str, Any]:
    """
...
async def update_sheet_headers(
    db: Session, config_id: int, sheet_type: str, headers: List[str]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if sheet_type == "match":
            ...
            return {"status": "error", "message": f"Invalid sheet type: {sheet_type}"}
        ...
        return {"status": "error", "message": f"Error updating sheet headers: {str(e)}"}
