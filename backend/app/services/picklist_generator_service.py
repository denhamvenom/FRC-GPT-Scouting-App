...
# Allow model configuration via env
...
class PicklistGeneratorService:
    ...
    def __init__(self, unified_dataset_path: str):
        ...
        # Store model and token encoder
        self.openai_model = OPENAI_MODEL
        try:
            self.token_encoder = tiktoken.encoding_for_model(self.openai_model)
        except Exception:
    ...
    def _select_model(self, team_count: int) -> str:
        """Select an appropriate model based on team count."""
        if team_count > 40 and self.openai_model == "gpt-4.1-nano":
            logger.info("Switching to gpt-4-turbo for larger prompt")
            return "gpt-4-turbo"
    ...
    async def generate_picklist(
        ...
        Args:
            ...
            final_rerank: If True, perform a final GPT pass to re-rank the merged batches
        ...
        try:
            ...
            if False:  # Was: if use_batching and len(teams_data) > batch_size:
                ...
                # Optional final re-ranking for cross-batch consistency
                if final_rerank:
                    logger.info("\n----- Final Re-ranking Across Batches -----")
                    picklist = await self._rerank_combined_picklist(
                        picklist,
                        your_team_number,
                        pick_position,
                    )
                ...
                while retry_count <= max_retries:
                    try:
                        response = client.chat.completions.create(
                            model=self._select_model(len(team_numbers)),
                            messages=messages,
    ...
    async def rank_missing_teams(
        ...
        try:
            ...
            while retry_count <= max_retries:
                try:
                    response = client.chat.completions.create(
                        model=self._select_model(len(teams_data)),
                        messages=messages,
                        temperature=0.2,
                        response_format={"type": "json_object"},
    ...
    async def _process_team_batch(
        ...
        try:
            ...
            while retry_count <= max_retries:
                try:
                    response = client.chat.completions.create(
                        model=self._select_model(len(batch_team_numbers)),
                        messages=messages,
                        temperature=0.2,  # Lower temperature for more consistent results
                        response_format={"type": "json_object"},
...
"""
    ...
    async def _rerank_combined_picklist(
        self,
        picklist: List[Dict[str, Any]],
        your_team_number: int,
        pick_position: str,
    ) -> List[Dict[str, Any]]:
        """Re-rank a merged picklist using GPT for cross-batch consistency."""

        system_prompt = (
            "You are GPT-4.1, an FRC strategist. Given PICKLIST_TO_RERANK, "
            "return a final ranking in the same compact JSON format."
        )
        user_prompt = f"PICKLIST_TO_RERANK = {json.dumps(picklist)}"
        ...
        model = self._select_model(len(picklist))
        try:
            response = client.chat.completions.create(
                model=model,
                messages=messages,
            ...
            teams_data = self._prepare_team_data_for_gpt()
            reranked = self._parse_response_with_index_mapping(data, teams_data)
            if reranked:
                return reranked
        except Exception as e:
