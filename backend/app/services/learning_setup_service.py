...
async def start_learning_setup(
    year: int, manual_file: Optional[UploadFile] = None, event_key: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    json_data_dir = os.path.join(
        data_dir, f"manual_processing_data_{year}"
    )  # Per-year directory for JSONs
        ...
        try:
            ...
            if manual_file.filename and saved_manual_path:  # Ensure we have critical info
                ...
                try:
                    ...
                    db_manual = (
                        db.query(GameManual)
                        .filter_by(year=year, sanitized_filename_base=s_filename_base)
                        .first()
                    )
                    ...
                    if db_manual:
                        ...
                        db_manual.original_filename = (
                            original_filename  # In case it changed subtly but base is same
                        )
                        print(
                            f"Updating existing GameManual DB record for {original_filename}, year {year}"
                        )
                    else:
                        # Create new record
                        ...
                        db.add(db_manual)
                        print(
                            f"Creating new GameManual DB record for {original_filename}, year {year}"
                        )
                    ...
                    print(
                        f"GameManual DB record ID {db_manual.id} processed for {original_filename}"
                    )

                except Exception as db_e:
                    print(
                        f"Database error processing GameManual for {manual_file.filename}: {db_e}"
                    )
    ...
    if manual_file:  # manual_url is removed from this condition
        ...
        try:
            ...
            if manual_text:
                ...
                if existing_manual_text and existing_game_analysis:
                    manual_info["text_length"] = len(
                        existing_manual_text.get("relevant_sections", "")
                    )  # Consider if this is still valid
        ...
        if existing_manual_info and existing_manual_text and existing_game_analysis:
            ...
            manual_info["game_name"] = existing_manual_info.get("game_name")
            manual_info["using_cached_manual"] = True
            manual_info["sections_extracted"] = existing_manual_info.get(
            ...
            if "processed_filename" in existing_manual_info:  # Current check
                manual_info["processed_filename_from_cache"] = existing_manual_info.get(
                    "processed_filename"
                )
            ...
            if existing_manual_info and "processed_filename_from_cache" in manual_info:
                cached_filename = manual_info[
                    "processed_filename_from_cache"
                ]  # This is original_filename
                if cached_filename:
                    ...
                    try:
                        ...
                        if (
                            db_manual
                            and db_manual.toc_json_path
                            and os.path.exists(db_manual.toc_json_path)
                        ):
                            ...
                            # db.commit()
                            print(
                                f"Loaded cached ToC from DB-verified path: {db_manual.toc_json_path}"
                            )
                        elif db_manual:
                            print(
                                f"DB record for {cached_filename} found, but ToC path '{db_manual.toc_json_path}' missing or invalid."
                            )
                            manual_info["toc_found"] = False
                        else:  # Fallback to old logic if not in DB for some reason (should be rare)
                            print(
                                f"No DB record for {cached_filename}, year {year}. Falling back to legacy cache check."
                            )
            ...
            cache["manual_text"] = existing_manual_text.get(
                "relevant_sections", ""
            )  # Or full text if that's what's cached
